...
/// WorldTime의 엔티티를 관리하는 코어 데이터 객체
final class WorldTimeDataManager: CoreDataManaged {
    typealias Model = WorldTimeDTO
    typealias Entity = WorldTimeEntity
    
    private lazy var context = self.persistentContainer.viewContext
    
    static let shared = WorldTimeDataManager() // 싱글톤
    private init() {}
